<template>
  <el-form :model="theme" label-width="auto" style="max-width: 100%">
    <el-form-item :label="t('settings.writingMode')">
      <el-radio-group v-model="theme.writingMode" size="small" style="flex-wrap: nowrap">
        <el-radio-button value="horizontal-tb" border> {{ t('settings.horizontal') }} </el-radio-button>
        <el-radio-button value="vertical-rl" border> {{ t('settings.vertical') }} </el-radio-button>
      </el-radio-group>
    </el-form-item>
    ...
    <el-form-item :label="t('settings.backgroundColor')">
    ...
    <el-form-item :label="t('settings.lineHeight')">
    ...
    <el-form-item :label="t('settings.textAlign')">
      <el-select v-model="theme.textAlign" class="font-select" width="50" size="small">
        <el-option v-for="item in textAlignList" :key="item" :label="t(`settings.align.${item}`)" :value="item" />
      </el-select>
    </el-form-item>
    ...
    <el-form-item>
      <el-button type="primary" @click="restore">{{ t('settings.restoreDefaults') }}</el-button>
    </el-form-item>
  </el-form>
</template>
